...
export class ShiftService {
  ...
  private mapToModel(row: any): Shift {
    return {
      id: row.id,
      name: row.name,
  ...
  private mapToDb(shift: Shift): any {
    return {
      id: shift.id,
      name: shift.name,
      ...
      updated_at: shift.updatedAt ?? null
    };
  }
  ...
  getById(id: string): Observable<Shift | undefined> {
    return from(
      this.supabaseService.client
        .from(this.tableName)
        .select('*')
  ...
  create(shift: Shift): Observable<Shift> {
    const dbRecord = this.mapToDb(shift);
    return from(
      this.supabaseService.client
        .from(this.tableName)
        .insert(dbRecord)
