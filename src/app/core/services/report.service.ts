...
/**
 * Optional presentation overrides for {@link ReportService.drawTable}. The
 * DEFAULT (all fields undefined) reproduces the existing dark print style so
 * non-daily reports keep their current look; the Daily Operational Report
 * passes light, black-and-white friendly values.
 */
export interface PdfTableVisuals {
  fontSize?: number;
  ...
  /** When true, zebra rows are suppressed (white rows only). */
  disableAlternateRows?: boolean;
  /**
   * When false, the autoTable per-page footer ("Generated by TPMS / Page n")
   * is NOT drawn; the caller is then responsible for footers (e.g. the daily
   * report's page-independent "Page X of Y" footer).
   */
  footer?: boolean;
}
...
export class ReportService {
  ...
  // ── Black & white / low-ink palette for the Daily Operational Report ──────
  // Near-black ink, very light gray fills/rules only — no heavy toner areas.
  private readonly BW_TEXT: [number, number, number] = [35, 35, 35];
  private readonly BW_MUTED: [number, number, number] = [90, 90, 90];
  private readonly BW_RULE: [number, number, number] = [205, 205, 205];
  private readonly BW_HEADER_FILL: [number, number, number] = [240, 240, 240];
  private readonly BW_SECTION_LINE: [number, number, number] = [150, 150, 150];
  ...
  private drawTable(doc: jsPDF, head: string[][], body: string[][], startY: number, columnStylesOverride?: Record<number, object>, avoidRowSplit = false, visuals: PdfTableVisuals = {}): number {
    ...
    autoTable(doc, {
      ...
      styles: {
        fontSize: v.fontSize ?? 7.5,
        cellPadding: v.cellPadding ?? 2.5,
        textColor: v.textColor ?? this.TEXT_DARK,
        lineColor: v.lineColor ?? this.TABLE_BORDER,
        lineWidth: v.lineWidth ?? 0.2,
        font: this.ARABIC_FONT
      },
      headStyles: {
        fillColor: v.headerFillColor ?? this.TABLE_HEADER_BG,
        textColor: v.headerTextColor ?? this.WHITE,
        fontStyle: 'bold',
        fontSize: v.headerFontSize ?? 7.5,
        cellPadding: 3,
        halign: 'center'
      },
      alternateRowStyles: v.disableAlternateRows ? { fillColor: this.WHITE } : { fillColor: this.TABLE_ALT_ROW },
      bodyStyles: {
        halign: 'left',
        valign: 'middle'
      },
      ...
      ...(v.footer === false ? {} : { didDrawPage: (data: { pageNumber: number }) => this.addPageFooter(doc, data.pageNumber) })
    });
  ...
  // ─── Daily Operational Report: light black-and-white presentation ─────────

  /** Light, low-ink table visuals used by every Daily table. Omitted params keep the existing dark style for other report types. */
  private dailyVisuals(fontSize = 7, compact = false): PdfTableVisuals {
    return {
      fontSize,
      headerFontSize: fontSize,
      headerFillColor: this.BW_HEADER_FILL,
      headerTextColor: this.BW_TEXT,
      textColor: this.BW_TEXT,
      lineColor: this.BW_RULE,
      lineWidth: 0.18,
      cellPadding: compact ? 1.4 : 1.8,
      theme: 'grid',
      disableAlternateRows: true,
      footer: false
    };
  }

  /** Explicit per-column widths (sum = 269 mm usable) + right alignment for numeric columns. */
  private dailyColStyles(widths: number[], rightAligned: number[]): Record<number, object> {
    const styles: Record<number, object> = {};
    widths.forEach((w, i) => {
      const style: { cellWidth: number; halign: 'left' | 'right' } = { cellWidth: w, halign: 'left' };
      if (rightAligned.includes(i)) {
        style.halign = 'right';
      }
      styles[i] = style;
    });
  ...
  /** Compact B/W letterhead: TPMS / system name left, report title + ref right, period + generated, thin rule. */
  private drawDailyHeader(doc: jsPDF, p: ReportParams, title: string): number {
    const pageW = doc.internal.pageSize.getWidth();
    const m = 14;
    ...
    doc.setDrawColor(...this.BW_SECTION_LINE);
  ...
  /** Light-bordered KPI/metric cells: bold value + small muted label, used for the KPI ribbon and Quality summary. */
  private drawDailyCells(doc: jsPDF, items: { label: string; value: string }[], startY: number, colsPerRow: number, rowHeight: number, valueSize: number): number {
    const pageW = doc.internal.pageSize.getWidth();
    const margin = 14;
    const gap = 2;
    const cellW = (pageW - margin * 2 - (colsPerRow - 1) * gap) / colsPerRow;
    ...
    items.forEach((item, i) => {
      ...
      const cy = startY + row * (rowHeight + gap);
    ...
    const rows = Math.ceil(items.length / colsPerRow);
    return startY + rows * (rowHeight + gap) + 2;
  }
  ...
  private drawDailySectionTitle(doc: jsPDF, title: string, y: number): number {
    ...
    doc.setDrawColor(...this.BW_RULE);
    doc.setLineWidth(0.2);
    doc.line(14, y + 1.6, pageW - 14, y + 1.6);
    return y + 6.5;
  }

  /** Real page numbers: redrawn on every page so each shows "Page X of Y". */
  private drawDailyFooters(doc: jsPDF, label: string): void {
    const pageW = doc.internal.pageSize.getWidth();
    const pageH = doc.internal.pageSize.getHeight();
  ...
  private buildPdfDaily(doc: jsPDF, p: ReportParams): void {
    ...
    if (!this.hasOperationalData(p)) {
      y = this.drawNoData(doc, y);
      this.drawDailyFooters(doc, reportLabel);
      return;
    }
